                                  ...
                                  const real_T params[30], real_T grad[50])
{
  ...
  _mm_storeu_pd(&b_y[0], _mm_mul_pd(b_r1, b_r1));
  b_r1 = _mm_loadu_pd(&r2[0]);
  r = _mm_sub_pd(b_r1, r);
  _mm_storeu_pd(&c_y[0], _mm_mul_pd(r, r));
  r = _mm_loadu_pd(&r3[0]);
  b_r1 = _mm_sub_pd(r, b_r1);
  _mm_storeu_pd(&d_y[0], _mm_mul_pd(b_r1, b_r1));
  ...
  l_u = 2.0 * params[27] * b_a;
  grad_us_idx_0 = l_u * v_s_tmp;
  grad_us_idx_1 = l_u * w_s_tmp;
  ...
  for (n = 0; n < i1; n++) {
    ...
    l_u = v_n - v_s_tmp;
    grad_us_idx_0 -= l_u;
    grad_us_idx_1_tmp = W[(int32_T)u + 1] - w_s_tmp;
    grad_us_idx_1 -= grad_us_idx_1_tmp;
    /*  Avaliação Suavizada Ponto a Ponto para CBF */
    st.site = &m_emlrtRSI;
    P_k = calc_point_p2s_penalty(
        &st, &(*(real_T(*)[3]) & X_hist_data[3 * ((int32_T)b_n - 1)])[0],
        &params[10], a__1);
    st.site = &n_emlrtRSI;
    P_next = calc_point_p2s_penalty(
        &st, &(*(real_T(*)[3]) & X_hist_data[3 * (int32_T)b_n])[0], &params[10],
        gP_seg0_r1);
