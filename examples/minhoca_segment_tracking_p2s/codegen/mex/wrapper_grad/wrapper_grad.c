                  ...
                  const real_T params[36], real_T grad[50])
{
  ...
  r1[0] = u[(int8_T)(d + 5.0) - 1];
  r1[1] = u[(int8_T)((d + 5.0) + 1.0) - 1];
  ...
  P_k = (int32_T)muDoubleScalarFloor(params[7] + 1.0);
  if (params[7] + 1.0 != P_k) {
  ...
  /*  =========================================================================
   */
  v1 = phi_func(params[32] - params[0], &g_corridor);
  v_n = phi_func(params[0] - params[33], &g_corridor);
  v3 = phi_func(params[34] - params[1], &g_corridor);
  v4 = phi_func(params[1] - params[35], &g_corridor);
  ...
  for (n = 0; n < i1; n++) {
    ...
    calc_point_p2s_penalty(&x_k[0], &params[10], a__2);
    ...
    calc_point_p2s_penalty(&x_next[0], &params[10], a__2);
    x_k[0] = x_next[0];
    x_k[1] = x_next[1];
    x_k[2] = x_next[2];
  ...
  calc_segment_p2s_penalty(r3, &params[3], &params[10], gP_seg3_r3, a__2);
  ...
  g_corridor = 2.0 * params[27] * (params[8] * params[8]);
  ...
  b_r3 = _mm_loadu_pd(&xs[0]);
  b_r1 = _mm_loadu_pd(&gP_seg0_xs[0]);
  r4 = _mm_set1_pd(
      2.0 * (params[29] + params[30] * muDoubleScalarExp(
                                           -params[31] *
                                           (0.25 * (((v1 + v_n) + v3) + v4)))));
  r5 = _mm_set1_pd(params[28]);
  r6 = _mm_set1_pd(-1.0);
  _mm_storeu_pd(&grad_xs[0],
                _mm_add_pd(_mm_sub_pd(_mm_mul_pd(r, r6),
                                      _mm_mul_pd(r4, _mm_sub_pd(b_r2, b_r3))),
                           _mm_mul_pd(r5, b_r1)));
  r7 = _mm_loadu_pd(&r2[0]);
  r = _mm_loadu_pd(&gP_seg0_r1[0]);
  b_r1 = _mm_loadu_pd(&gP_seg1_r1[0]);
  r8 = _mm_set1_pd(2.0);
  _mm_storeu_pd(
      &xs[0],
      _mm_add_pd(
          _mm_mul_pd(r4,
                     _mm_sub_pd(_mm_sub_pd(_mm_mul_pd(r8, b_r2), b_r3), r7)),
          _mm_mul_pd(r5, _mm_add_pd(r, b_r1))));
  r9 = _mm_loadu_pd(&r3[0]);
  r = _mm_loadu_pd(&gP_seg1_r2[0]);
  b_r1 = _mm_loadu_pd(&gP_seg2_r2[0]);
  _mm_storeu_pd(
      &r1[0],
      _mm_add_pd(
          _mm_mul_pd(r4, _mm_sub_pd(_mm_sub_pd(_mm_mul_pd(r8, r7), b_r2), r9)),
          _mm_mul_pd(r5, _mm_add_pd(r, b_r1))));
  r = _mm_loadu_pd(&gP_seg2_r3[0]);
  b_r1 = _mm_loadu_pd(&gP_seg3_r3[0]);
  _mm_storeu_pd(
      &r3[0],
      _mm_add_pd(_mm_mul_pd(r4, _mm_sub_pd(_mm_sub_pd(_mm_mul_pd(r8, r9), r7),
  ...
  for (n = 0; n < i1; n++) {
    ...
    uint32_T b_u;
    uint32_T u1;
    v3 = params[7] - (real_T)n;
    if (v3 != (int32_T)muDoubleScalarFloor(v3)) {
      emlrtIntegerCheckR2012b(v3, &b_emlrtDCI, &st);
    }
    if (((int32_T)v3 < 1) || ((int32_T)v3 > i2)) {
      emlrtDynamicBoundsCheckR2012b((int32_T)v3, 1, i2, &e_emlrtBCI, &st);
    }
    if (((int32_T)(v3 + 1.0) < 1) || ((int32_T)(v3 + 1.0) > i2)) {
      emlrtDynamicBoundsCheckR2012b((int32_T)(v3 + 1.0), 1, i2, &d_emlrtBCI,
                                    &st);
    }
    b_u = (uint32_T)((int32_T)v3 - 1) << 1;
    ...
    loop_ub = 3 * ((int32_T)v3 - 1);
    v1 = 2.0 * (X_hist_data[loop_ub] - xs_tmp);
    grad_xs[0] -= v1;
    grad_xs_tmp = 2.0 * (X_hist_data[loop_ub + 1] - b_xs_tmp);
    grad_xs[1] -= grad_xs_tmp;
    ...
    P_k = calc_point_p2s_penalty(&X_hist_data[loop_ub], &params[10], a__2);
    v4 = calc_point_p2s_penalty(&X_hist_data[3 * (int32_T)v3], &params[10],
                                gP_seg0_xs);
    g_corridor = (1.0 - gamma_safe) * -P_k - (-v4);
    x_next[0] = 0.0;
    ...
    v4 = X_hist_data[loop_ub + 2];
    g_corridor = muDoubleScalarCos(v4);
    v4 = muDoubleScalarSin(v4);
    ...
    b_r1 = _mm_loadu_pd(&r2[0]);
    _mm_storeu_pd(&r2[0], _mm_add_pd(b_r1, _mm_mul_pd(r, r9)));
    r = _mm_loadu_pd(&b_Ts[4]);
    b_r1 = _mm_loadu_pd(&r2[0]);
    _mm_storeu_pd(&r2[0], _mm_add_pd(b_r1, _mm_mul_pd(r, b_r2)));
    a__2[0] = grad_us_idx_0_tmp;
    a__2[1] = grad_us_idx_1_tmp;
    r = _mm_loadu_pd(&r2[0]);
    b_r1 = _mm_loadu_pd(&a__2[0]);
  ...
  for (n = 0; n < 24; n++) {
    ...
    loop_ub = (n + 1) << 1;
    /*  v_{i+1} - v_i */
    g_corridor = 0.4 * (u[loop_ub] - u[loop_ub - 2]);
    grad_smooth[loop_ub - 2] -= g_corridor;
    grad_smooth[loop_ub] += g_corridor;
    /*  Derivada em relação a w */
    /*  w_{i+1} - w_i */
    g_corridor = 0.4 * (u[loop_ub + 1] - u[loop_ub - 1]);
    grad_smooth[loop_ub - 1] -= g_corridor;
    grad_smooth[loop_ub + 1] += g_corridor;
    if (*emlrtBreakCheckR2012bFlagVar != 0) {
  ...
  for (n = 0; n <= 48; n += 2) {
    ...
    b_r1 = _mm_loadu_pd(&grad_smooth[n]);
    _mm_storeu_pd(&grad[n], _mm_add_pd(r, b_r1));
  }
