                  ...
                  const real_T params[30], real_T grad[50])
{
  ...
  grad_us_idx_0_tmp = (int32_T)muDoubleScalarFloor(params[7] + 1.0);
  if (params[7] + 1.0 != grad_us_idx_0_tmp) {
  ...
  grad_us_idx_0_tmp = 2.0 * params[27] * (params[8] * params[8]);
  grad_us_idx_0 = grad_us_idx_0_tmp * v_s;
  grad_us_idx_1 = grad_us_idx_0_tmp * w_s;
  ...
  for (n = 0; n < i1; n++) {
    ...
    b_grad_us_idx_0_tmp = v_n - v_s;
    grad_us_idx_0 -= b_grad_us_idx_0_tmp;
    grad_us_idx_1_tmp = 2.0 * (u[(int32_T)b_u + 1] - w_s);
    ...
    g_corridor = calc_point_p2s_penalty(&X_hist_data[3 * (int32_T)b_n],
                                        &params[10], gP_seg0_xs);
    g_corridor = (1.0 - gamma_safe) * -grad_us_idx_0_tmp - (-g_corridor);
    x_next[0] = 0.0;
    ...
    grad_us_idx_0_tmp = X_hist_data[loop_ub + 2];
    g_corridor = muDoubleScalarCos(grad_us_idx_0_tmp);
    grad_us_idx_0_tmp = muDoubleScalarSin(grad_us_idx_0_tmp);
    grad_x_next_cbf[0] += x_k[0];
    grad_x_next_cbf[1] += x_k[1];
    grad_x_next_cbf[2] += x_k[2];
    dv[2] = -Ts * v_n * grad_us_idx_0_tmp;
    ...
    a__1[0] = b_grad_us_idx_0_tmp;
    a__1[1] = grad_us_idx_1_tmp;
    r = _mm_loadu_pd(&r2[0]);
    b_r1 = _mm_loadu_pd(&a__1[0]);
  ...
  for (n = 0; n < 24; n++) {
    ...
    loop_ub = (n + 1) << 1;
    /*  v_{i+1} - v_i */
    grad_us_idx_0_tmp = 0.4 * (u[loop_ub] - u[loop_ub - 2]);
    grad_smooth[loop_ub - 2] -= grad_us_idx_0_tmp;
    grad_smooth[loop_ub] += grad_us_idx_0_tmp;
    /*  Derivada em relação a w */
    /*  w_{i+1} - w_i */
    grad_us_idx_0_tmp = 0.4 * (u[loop_ub + 1] - u[loop_ub - 1]);
    grad_smooth[loop_ub - 1] -= grad_us_idx_0_tmp;
    grad_smooth[loop_ub + 1] += grad_us_idx_0_tmp;
    if (*emlrtBreakCheckR2012bFlagVar != 0) {
