...
static emlrtRSInfo m_emlrtRSI = {
    ...
    "C:\\Users\\henri\\Desktop\\robotarium_ws\\robotarium-matlab-"
    "simulator\\examples\\cbf_caixa\\build_robot_cbf_experiment.m" /* pathName
                                                                    */
};
                    ...
                    const real_T params[17])
{
  ...
  for (n = 0; n < i1; n++) {
    ...
    l_u = ((l_u + ((2.0 * (tmp2 * tmp2 + cost * cost) + 0.5 * (v_n * v_n)) +
                   0.01 * (w_n * w_n))) +
  ...
  /*  5. Custo total */
  cost = (l_u + 2.0 * (x_pos_k * x_pos_k + y_pos_k * y_pos_k)) +
