...
% (Formulação com Mapeamento de Estado Estacionário Injetado)
% =========================================================================
clear; clc; close all;
...
% TAMANHO DO VETOR ATUALIZADO (Removemos as 2 posições de u_s)
nW = 2*N + 8; 

W_max = [repmat([v_max; w_max], N, 1);
         3; 2;           % xs (que atua como ys)
         3; 2;           % r1
         3; 2;           % r2
         3; 2];          % r3
...
if exist('wrapper_cost_mex', 'file') == 3 && exist('wrapper_grad_mex', 'file') == 3
    ...
    disp('Rodando versão nativa .m. Lembre-se de compilar (codegen) com o novo tamanho (29)!');
...
hist_us = zeros(2, n_steps); % Mantido para plot, mas usaremos [0;0]
...
for k = 1:n_steps
    ...
    % PARÂMETROS ATUALIZADOS: Vetor de tamanho 29 (sem eta_eq)
    params = [X_k; x_ref_current; eta_safe; gamma_safe; N; Ts; r_rob; ...
        blocks_params; eta_term; mu_safe; kappa_s];
    ...
    hist_us(:, k) = [0; 0]; % Estado estacionário exato
    ...
    r1_opt = w_opt(2*N+3:2*N+4);
    r2_opt = w_opt(2*N+5:2*N+6);
    r3_opt = w_opt(2*N+7:2*N+8);
    
    % =====================================================================
    % EXTRAÇÃO DE CUSTOS PARA PLOTAGEM
    % =====================================================================
    Q_p_plot = 5; R_v_p_plot = 1; R_w_p_plot = 0.01; h_p_plot = 0.05;
    c_stage = 0; c_cbf = 0;
    
    for i = 1:N
        v_p = w_opt((i-1)*2 + 1); w_p = w_opt((i-1)*2 + 2);
        % Custo de Estágio (considerando vs=0, ws=0)
        c_stage = c_stage + Q_p_plot * norm(X_pred(1:2, i) - xs_opt)^2 + ...
                  R_v_p_plot * (v_p - 0)^2 + R_w_p_plot * (w_p - 0)^2;
        % Penalidade CBF
    ...
    hist_norm_term(k)     = norm(X_pred(1:2, N+1) - xs_opt);
    % =====================================================================
    ...
    w_init = [U_warm; xs_opt; r1_opt; r2_opt; r3_opt];
...
%% 7. Plotagem da Evolução dos Custos
...
%% 8. Plotagem da Evolução do Custo Total
hist_cost_total = hist_cost_stage + hist_cost_cbf + hist_cost_term + ...
                  hist_cost_elastic + hist_cost_geofence;
