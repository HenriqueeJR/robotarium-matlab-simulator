                                  ...
                                  const real_T params[17], real_T grad[40])
{
  ...
  for (n = 0; n < x_pos_next_tmp; n++) {
    ...
    b_a = x_k[1] - params[4];
    l_u = ((l_u + ((1.8 * (P2_next * P2_next + b_a * b_a) + 0.5 * (v_n * v_n)) +
                   0.001 * (w_n * w_n))) +
           eta_safe * (a * a)) +
  ...
  cost = l_u + 1.8 * (dy2_next * dy2_next + P2_next * P2_next);
  p_n[0] = 3.6 * dy2_next;
  p_n[1] = 3.6 * P2_next;
  p_n[2] = 0.0;
  ...
  for (n = 0; n < i; n++) {
    ...
    p_n[2] += gP1_n[2];
    gP1[0] += 3.6 * (x_pos_k_tmp - params[3]);
    gP1[1] += 3.6 * (y_pos_k_tmp - params[4]);
