...
static emlrtRSInfo emlrtRSI = {
    ...
    "simulator\\examples\\minhoca_segment_tracking\\wrapper_cost.m" /* pathName
                                                                     */
};
                    ...
                    const real_T params[30])
{
  ...
  for (n = 0; n < i1; n++) {
    ...
    b_a = x_k[0] - xs_tmp;
    P_next = x_k[1] - b_xs_tmp;
    tmp2 = v_n - v_s_tmp;
    cost = w_n - w_s;
    l_u = (l_u + ((5.0 * (b_a * b_a + P_next * P_next) + 0.5 * (tmp2 * tmp2)) +
                  cost * cost)) +
          params[5] * (P_k * P_k);
    x_k[0] = x_next[0];
    x_k[1] = x_next[1];
  ...
  P_next = calc_segment_p2s_penalty(xs, r1, &params[10], a, gP_seg0_r1);
  /*  Segmento 2: r1 até r2 */
  P_c_n = calc_segment_p2s_penalty(r1, r2, &params[10], gP_seg0_r1, a);
  /*  Segmento 3: r2 até r3 */
  P_k = calc_segment_p2s_penalty(r2, r3, &params[10], a, gP_seg0_r1);
  /*  Segmento 4: r3 até r4 (x_ref) */
  /*  Nota: o gradiente para r4 é computado mas descartado via ~, pois r4 é
   * parâmetro constante. */
  b_a = calc_segment_p2s_penalty(r3, &params[3], &params[10], a, gP_seg0_r1);
  /*  Pesos de suavização  */
  cost = u[0];
  for (k = 0; k < 24; k++) {
    tmp2 = cost;
